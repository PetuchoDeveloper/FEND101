...
async def listar_productos(session: aiohttp.ClientSession, categoria=None, orden=None):
    ...
    try:
        ...
        raise TimeoutError(f"La petición a listar_productos tardó más de {TIMEOUT} segundos")
...
async def obtener_categorias(session: aiohttp.ClientSession) -> list:
    ...
    try:
        ...
        raise TimeoutError(f"La petición a obtener_categorias tardó más de {TIMEOUT} segundos")
    except aiohttp.ClientConnectorError as e:
        raise ConexionError(f"No se pudo conectar con el servidor: {e}")
    except asyncio.CancelledError:
        raise EcoMarketError("La tarea fue cancelada")
...
async def obtener_perfil(session: aiohttp.ClientSession) -> dict:
    ...
    try:
        ...
        raise TimeoutError(f"La petición a obtener_perfil tardó más de {TIMEOUT} segundos")
    except aiohttp.ClientConnectorError as e:
        raise ConexionError(f"No se pudo conectar con el servidor: {e}")
    except asyncio.CancelledError:
        raise EcoMarketError("La tarea fue cancelada")
